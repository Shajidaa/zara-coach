    ...
    <>
      <nav className="sticky top-0 z-[100] w-full bg-black border-b border-white/10">
        <div className="max-w-[1440px] mx-auto h-20 px-4 md:px-8 flex items-center justify-between">
          ...
          {/* LEFT: Logo Area - Fixed width container to prevent squishing */}
          ...
          {/* CENTER: Desktop Menu - Completely hidden on tablet/mobile */}
          <div className="hidden lg:flex items-center gap-8">
            {navItems.map((item) => (
              <Link 
                key={item.name} 
                href={item.href}
                className="text-white/70 hover:text-[#D4AF37] text-[11px] font-bold uppercase tracking-widest transition-all"
              >
                {item.name}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-5">
            {/* Phone - Hidden on small mobile, shown on tablet/desktop */}
            ...
            {/* Hamburger Button - High visibility */}
            <button 
          ...
          <div className="flex flex-col p-6 gap-6 items-center text-center">
            {navItems.map((item) => (
              <Link 
                ...
                className="text-white text-lg font-light uppercase tracking-[0.2em] hover:text-[#D4AF37]"
              >
                {item.name}
              </Link>
            ))}
